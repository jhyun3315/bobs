...
public interface CommunityRepository extends JpaRepository<Community, Long> {
    ...
    public Page<Community> findByUser(@Param("userId") Long user_id, @PageableDefault(size = 10 )Pageable pageable);

    @Transactional(readOnly = true)
    @Query(value = "SELECT * FROM community WHERE community_deleted = 0 ORDER BY community_created DESC", nativeQuery = true)
    public Page<Community> findAll(@PageableDefault(size = 10) Pageable pageable);
